...
\begin{document}
  ...
  to approximate the posterior, rather than a Laplacian approximation of the posterior. Following
  the presentation in section 8.4.1, the posterior is given by
  \begin{equation}
    p(\mu, \ell | \mathcal{D}) = \frac1Z e ^ {-E(\mu, \ell)}
  \end{equation}
  where
  \[
    E(\mu, \ell) = n \log \sigma + \frac{n}{2\sigma^2}\left(s^2 + (\overline{x} - \mu) ^ 2 \right)
  \]
  ...
  \begin{align*}
    ...
    \frac{\partial}{\partial \ell} \left[n \ell + e ^ {-2\ell}\frac n2
    \left(s^2 + (\overline{x} - \mu) ^ 2\right)\right] \\
    &= n - e ^ {-2\ell} n \left(s^2 + (\overline{x} - \mu) ^ 2 \right) \\
    &= n - \frac{n \left(s^2 + (\overline{x} - \mu) ^ 2 \right)}{\sigma^2}. \\
  \end{align*}
  ...
  \[
    \mathbb{KL}(q_i||\tilde{p}_i) = \sum\limits_{x\in\{-1,1\}}q_i(x)\log\frac{q_i(x)}{\tilde{p}_i(x)}
  \]
  ...
  \exercise
  \begin{align*}
    \mathbb{KL}(p||q) &= \sum\limits_{x,y} p(x,y) \log \frac{p(x,y)}{q(x)q(y)} \\
    &= \sum\limits_{x,y} p(x,y) ( \log p(x,y) - \log q(x) - \log q(y) -) \\
    & = \sum\limits_{x,y} p(x,y)\log p(x,y) - \sum\limits_y p(y)\log q(y) - \sum\limits_x p(x)\log q(x) \\
  \end{align*}
  We minimize this by setting $q(x) = p(x)$ and $q(y) = p(y)$.

  Following sections 21.2.2,  $p(x,y) = 0  \implies q(x)q(y) = 0$. It follows that the three local
  minima are
  \begin{align*}
    &q(x) = q(y) = [0.5, 0.5, 0, 0] \\
    &q(x) = q(y) = [0, 0, 1, 0] \\
