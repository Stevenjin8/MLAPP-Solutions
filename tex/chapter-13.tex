...
\begin{document}
  ...
  \section*{Exercises}
    ...
    \begin{align*}
      ...
      &= \frac{\partial }{\partial w_k}
        \sum (\mathbf{x}_i^T \mathbf{w} - y_i) ^ 2 \\
    ...
    \begin{align*}
      ...
      - \sum x_{ik} ^ 2 w_k &= 0 \\
      \mathbf{r}_k^T \mathbf{x}_{:k} - ||\mathbf{x}_{:k}||_2^2 w_k &= 0 \\
      \hat{w}_k &= \frac{
  ...
  \exercise
    ...
    line is linear regression ad the dashed line is ridge regression.
    ...
    $\frac12$. Using results from part \textbf{a}, $a_k=2$ and $a_k + 2\lambda_2 = 4$. Thus,
    $\lambda_2 = 1$.
 ...
 \begin{align*}
    p(\boldsymbol{\gamma}|\boldsymbol{\alpha})
    &= \pi_0 ^ {||\boldsymbol{\gamma}||_0}
    (1 - \pi_0) ^ {D - ||\boldsymbol{\gamma}||_0}
 \end{align*}
 where $\pi_0 = \frac{\alpha_1}{\alpha_1 + \alpha_2}$. So, using a Beta prior is the same as using
 a fixed $\pi_0$.
  ...
  \exercise
    ...
    \begin{align*}
      ...
      &= \frac1{p(w_j)} \int \frac{1}{|w_j|} \frac{|w_j|}{2\tau^2_j} 
        \mathcal{N}(w_j|0, \tau^2_j)p(\tau^2_j)
      d\tau^2_j \\
      &= \frac1{p(w_j)} \frac{1}{|w_j|} \int \frac{d}{d|w_j|} \left[
        \mathcal{N}(w_j|0, \tau^2_j)
      \right]p(\tau^2_j)
      d\tau^2_j \\
      &= \frac1{p(w_j)} \frac{1}{|w_j|} \frac{d}{d|w_j|} \int 
        \mathcal{N}(w_j|0, \tau^2_j)
      p(\tau^2_j)
      d\tau^2_j \\
      &= \frac{1}{|w_j|} \frac1{p(w_j)} \frac{d}{d|w_j|} p(w_j) \\
      &= \frac{1}{|w_j|} \frac{d}{d|w_j|} \log p(w_j) \\
      &= \frac{\pi'(w_j)}{|w_j|}.
    \end{align*}
    We can further reduce this equation since $p(w_j) = Lap\left(w_j|0, \frac1\gamma\right)$. Also
    ...
    \begin{equation*}
      \mathbb{E}\left[\frac1{\tau^2}\right] = \frac\gamma{|w_j|}.
    \end{equation*}
    ...
    \end{equation*}
    And we can optimize with any gradient based method. We can see that the regularization term in
