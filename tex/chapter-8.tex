...
\begin{align*}
    ...
    &> 0.
\end{align*}
The strict inequality comes from the fact that $\mathbf{X}$ is full rank and
$\mathbf{p}_i$ is non-zero so at least one element of 
$\mathbf{a}_i = \mathbf{Xp}_i$ is non-zero.
Since $\mathbf{H}$ is a symmetric matrix with positive eigenvalues, it is
positive definite.
...
when $x_i=0$, $\frac{\partial J}{\partial w_i}\vert_{w_i=0}=0$ if
and only if $\frac{\partial \ell}{\partial x_i}\vert_{w_i=0}=0$ 
